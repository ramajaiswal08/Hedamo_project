...
export default function MeatPoultryPage() {
  ...
  const filtered = React.useMemo(() => {
    ...
    const base = meatProducts.filter(
      (p) => p.category.toLowerCase() === "meat-poultry"
    );
    return !q
      ? base
      : base.filter(
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        <header className="mb-8 space-y-3">
          <h1 className="text-balance text-2xl font-semibold tracking-tight">
            Meat & Poultry — Fresh Picks
          </h1>
          <img
            src="https://tse4.mm.bing.net/th/id/OIP.YqJXmJxzPAKv8NAsrLxlJAHaEO?pid=Api&P=0&h=180"
            alt="Meat & Poultry"
            className="w-full h-60 object-cover rounded-lg mb-4"
          />
          <p className="max-w-2xl text-pretty text-sm text-foreground/70">
            Explore fresh meat and poultry products. Tap a card to learn more,
            or expand with the arrow to preview images, specs, and features.
          </p>
